...
def pytorch2libtorch(model,
    ...
    if hasattr(model, 'forward_dummy'):
        ...
        # MMSeg >=1.0 uses unified forward(mode='tensor')
        original_forward = model.forward  # 保存原始 forward 引用
        def _forward_tensor(x, *args, **kwargs):
            return original_forward(x, mode='tensor')  # ✅ 不传 data_samples
        model.forward = _forward_tensor
